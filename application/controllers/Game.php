...
class Game extends CI_Controller
{
    ...
    public function main($id)
    {
        if (file_exists('./lobbys/' . $id . '.json')) {
            ...
            $this->template->load('template/template', 'game/game', ['id' => $id]);
        } else {
            redirect();
        }
    }
    ...
    public function playCard($id, $userName)
    {
        ...
        foreach ($lobbyData['players'][$userName]['hand'] as $number => $card) {
            if ($card['id'] == $_POST['id']) {
                ...
                switch (substr($card['name'], 1)) {
                    case 'plus4':
                        $lobbyData['cardDictate'] = ['order' => 'plus4', 'origin' => $userName, 'executed' => false];
                        break;
                    case 'plus2':
                        $lobbyData['cardDictate'] = ['order' => 'plus2', 'origin' => $userName, 'executed' => false];
                        break;
                    case "r":
                        $lobbyData['clockwise'] = ! $lobbyData['clockwise'];
                        break;
                    case "a":
                        if ($lobbyData['clockwise']) {
                            $lobbyData['turn']++;
                        } else {
                            $lobbyData['turn']--;
                        }
        ...
        if (count($lobbyData['players'][$userName]['hand']) < 1) {
            ...
            if ($lobbyData['clockwise']) {
                $lobbyData['turn'] = ($lobbyData['turn'] + 1) % $lobbyData['playerCount'];
            } else {
                $lobbyData['turn'] = ($lobbyData['turn'] + ($lobbyData['playerCount'] - 1)) % $lobbyData['playerCount'];
            }
    ...
    public function drawCard($id, $userName, $endTurn = 1)
    {
        ...
        $sendData = ['card' => $card, 'playable' => $cardCanBePlayed];
